...
>
	...
	/>
		...
		const expected = `import type { EmotionJSX } from '@emotion/react/types/jsx-namespace';
import { iconSize } from '@guardian/source-foundations';
import type { IconProps } from '../types';

export const SvgTest = ({ size }: IconProps): EmotionJSX.Element => {
	return (
		<>
		<svg
			viewBox="0 0 24 24"
...
`;
		expect(generateReactComponent('Test', svg, 'Test Label')).toBe(
			expected,
		);
	});
});
