...
- 905c5148: `TextArea`, `TextInput`, and `Select` active border colour persists when focused. `Radio` and `Checkbox` border changes on hover when unchecked and remains in the error state on hover when checked.
- 905c5148: Typography functions now include a `--source-text-decoration-thickness` CSS variable.
