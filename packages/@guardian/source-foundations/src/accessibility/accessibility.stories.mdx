...
import { FocusStyleManager } from '../utils/focus-style-manager';

<Meta
	title="Packages/source-foundations/Accessibility"
...
<ul>
	{Object.keys(FocusStyleManager).map((method) => (
		<li>
			<code>{`FocusStyleManager.${method}()`}</code>
		</li>
	))}
</ul>
...
Apply the `visuallyHidden` style to elements that need to be hidden from sighted users and displayed to screen reader users:
